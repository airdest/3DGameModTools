...
def get_pointlit_and_trianglelist_indices(input_ib_hash, root_vs):
    ...
    for index in range(len(indices)):
        ...
        ib_files = glob.glob(indices[index] + '-ib*txt')
        if len(ib_files) > 0:
...
def start_merge_files(input_ib_hash, part_name , root_vs):
    ...
    for index in range(len(ib_file_bytes)):
        ...
        output_vbname = "output/output-" + input_ib_hash + str(index) + "-vb0.txt"
        output_ibname = "output/output-" + input_ib_hash + str(index) + "-ib.txt"
...
if __name__ == "__main__":
    # setting work dir
    os.chdir("C:/Users/Administrator/Desktop/BH3Loader/FrameAnalysis-2023-02-10-110022/")
    if not os.path.exists('output'):
        os.mkdir('output')

    ib_hashs = {"2897bb22": "衣服", "1035d6b3":"胸前带子","cba1e5c5":"身体","60f6fd14":"检字","55269214":"MOSS"}
    for input_ib_hash in ib_hashs:
        start_merge_files(input_ib_hash, ib_hashs.get(input_ib_hash), root_vs="653c63ba4a73ca8b")
